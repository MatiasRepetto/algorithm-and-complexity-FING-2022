...
{
  bool existe = false;
  nat aux = cantidad(lstAsig);
  while (aux > 0 && !existe)
  {
    if (eid(primer_par(lstAsig)) == estudiante)
    {
      existe = true;
    }
    lstAsig = resto_pares(lstAsig);
    aux = aux - 1;
  }
  return existe;
}
  ...
  while (h <= m || h != 999)
  {
    ...
    else
    {
      ...
      for (nat i = 0; i <= m; i++)
      {
        ...
        {
          esMejorProposicion = false;
          break;
        }
        else if (ePrefs[e][i] == h)
        {
          esMejorProposicion = true;
          break;
        }
      }
